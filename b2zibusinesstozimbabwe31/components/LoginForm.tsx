...
import { Mail, Lock, ArrowLeft, AlertCircle, ShoppingBag } from "lucide-react"
...
export const LoginForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const storedAccount = localStorage.getItem("b2zi_account_data")

      if (storedAccount) {
        const account = JSON.parse(storedAccount)
        if (account.email.toLowerCase() === formData.email.toLowerCase()) {
          if (account.password === formData.password) {
            localStorage.setItem("b2zi_user", JSON.stringify({ name: account.name, email: account.email }))
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#2e3621] via-[#000000] to-[#2e3621] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#b1c98d] rounded-full opacity-20 blur-3xl animate-pulse"></div>
        <div
          className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-[#b1c98d] rounded-full opacity-20 blur-3xl animate-pulse"
          style={{ animationDelay: "1s" }}
        ></div>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-[#8ba970] rounded-full opacity-10 blur-3xl"></div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-white/95 backdrop-blur-sm p-8 md:p-10 rounded-3xl shadow-2xl border border-[#b1c98d]/20">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-black text-[#000000] mb-2">Welcome Back</h2>
            <p className="text-gray-600">Sign in to your B2Zi account</p>
          </div>

          {error && (
            <div className="mb-6 bg-red-50 border-2 border-red-200 text-red-700 px-4 py-3 rounded-xl flex items-start gap-3 animate-in slide-in-from-top-2">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white text-gray-500">New to B2Zi?</span>
            </div>
          </div>
        ...
        <p className="text-center text-[#b1c98d] text-sm mt-6">Shop quality products from trusted sellers</p>
      </div>
    </div>
  )
}
