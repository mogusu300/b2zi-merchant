    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom-4 duration-300">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-xl font-bold text-b2zi-black">Product Details</h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 p-6">
                ...
                {product.images.map((image, idx) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all ${
                      idx === currentImageIndex ? "border-b2zi-dark scale-105" : "border-gray-200 hover:border-gray-300"
                    }`}
                  >
          ...
          <div className="space-y-6">
            <div>
              <h1 className="text-3xl font-black text-b2zi-black mb-2">{product.name}</h1>
              {product.rating && (
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={`w-5 h-5 ${i < Math.floor(product.rating!) ? "fill-b2zi-light text-b2zi-light" : "text-gray-300"}`}
                      />
                    ))}
                  </div>
              ...
              <div className="text-4xl font-black text-b2zi-dark mb-4">${product.price.toFixed(2)}</div>
            </div>
            ...
            {product.colors && product.colors.length > 0 && (
              <div>
                <label className="block text-sm font-bold text-b2zi-dark mb-3">
                  Color: <span className="font-normal text-b2zi-black">{selectedColor}</span>
                </label>
                  ...
                  {product.colors.map((color) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg border-2 font-medium transition-all ${
                        selectedColor === color
                          ? "border-b2zi-dark bg-b2zi-dark text-white"
                          : "border-gray-200 hover:border-b2zi-light text-gray-700"
                      }`}
                    >
                      {color}
                    </button>
            ...
            {product.types && product.types.length > 0 && (
              <div>
                <label className="block text-sm font-bold text-b2zi-dark mb-3">
                  Type: <span className="font-normal text-b2zi-black">{selectedType}</span>
                </label>
                <div className="flex gap-3">
            ...
            {/* Quantity */}
            <div>
              <label className="block text-sm font-bold text-b2zi-dark mb-3">Quantity</label>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="w-10 h-10 border-2 border-gray-200 rounded-lg font-bold hover:border-b2zi-light transition-colors"
                >
                  -
                </button>
