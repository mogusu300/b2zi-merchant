...
export const ProductDetail: React.FC<ProductDetailProps> = ({ product, onClose, onAddToCart }) => {
  ...
  const colorGroup = useMemo(
    () => product.variantGroups?.find((g) => g.name.toLowerCase() === "color"),
    [product.variantGroups],
  )
  const otherVariantGroups = useMemo(
    () => product.variantGroups?.filter((g) => g.name.toLowerCase() !== "color") || [],
    [product.variantGroups],
  )
      ...
      <div className="bg-background rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl">
        ...
        <div className="p-6 md:p-8">
              ...
              {images.length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-3 top-1/2 -translate-y-1/2 p-2 bg-background hover:bg-secondary rounded-lg shadow-sm border border-border transition-all hover:scale-105"
                  >
                    <ChevronLeft className="w-5 h-5 text-foreground" />
                  </button>
                  <button
                    onClick={goToNextImage}
                    className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-background hover:bg-secondary rounded-lg shadow-sm border border-border transition-all hover:scale-105"
                  >
          ...
          <div className="space-y-6">
            {/* Category */}
            <div>
              <span className="inline-block px-2.5 py-1 bg-primary/10 text-primary rounded-md text-xs font-semibold uppercase tracking-wide">
                {product.category}
              </span>
            </div>
            ...
            {product.sellerName && (
              <div className="bg-secondary/40 rounded-xl p-4 border border-border/50">
                <div className="flex items-center gap-3">
                  ...
                  {!product.sellerAvatar && (
                    <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center border border-border">
                      <span className="text-base font-bold text-primary">{product.sellerName[0]}</span>
                    </div>
                  )}
                  <div className="flex-1">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Sold by</p>
                    <h3 className="text-sm font-bold text-foreground">{product.sellerName}</h3>
            ...
            {otherVariantGroups.map((variantGroup) => (
              <div key={variantGroup.name}>
                <label className="block text-sm font-semibold text-foreground mb-3">Choose {variantGroup.name}</label>
                <div className="flex gap-2 flex-wrap">
                  {variantGroup.values.map((value) => (
                    <button
                      key={value}
                      onClick={() => handleVariantSelect(variantGroup.name, value)}
                      className={`px-3.5 py-2 rounded-lg border-2 font-medium text-sm transition-all ${
            ...
            <div className="flex gap-3 pt-4">
              <div className="flex items-center border border-border rounded-lg bg-secondary/50">
                <button
                  ...
                  className="px-3 py-2 hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground"
                >
                  −
                </button>
